...
if ($vscodeExePath -and (Test-Path $vscodeExePath)) {
    ...
    if (Test-Path $workspaceFilePath) {
        Write-Host "Launching VSCode with workspace: $workspaceFilePath"
        Start-Process -FilePath $vscodeExePath -ArgumentList "`"$workspaceFilePath`""
    } else {
        Write-Warning "Workspace file not found: $workspaceFilePath"
    }
}
