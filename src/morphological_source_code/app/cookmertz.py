...
class ComplexByte:
    ...
    def __mul__(self, other: 'ComplexByte') -> 'ComplexByte':
        ...
        real_part = BinaryField.multiply(self.real, other.real) ^ BinaryField.multiply(
            self.imag, other.imag
        )
        imag_part = BinaryField.multiply(self.real, other.imag) ^ BinaryField.multiply(
...
class ByteWordFFT:
    ...
    def fft(self, data: List[int]) -> List[ComplexByte]:
        ...
        complex_data = [
            ComplexByte(x & 0xFF, (x >> 8) & 0xFF) if x > 255 else ComplexByte(x, 0)
            for x in data
        ]
...
class ByteWord:
    ...
    def propagate(self, steps: int = 1) -> List['ByteWord']:
        ...
        for step in range(steps):
            ...
            for i, freq_component in enumerate(spectrum):
                ...
                phase_shift = ComplexByte(
                    (freq_component.real + i) & 0xFF,
    ...
    def to_float(self) -> float:
        ...
        energy = sum(
            BinaryField.multiply(cb.real, cb.real)
